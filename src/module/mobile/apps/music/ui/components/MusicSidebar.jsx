...
const MusicSidebar = ({ activeCategory, onCategoryChange, isPlaying }) => {
  return (
    <div className="w-48 bg-[#f9f9fb] border-r border-zinc-200 flex flex-col shrink-0 min-w-0 overflow-y-auto">
      {SECTIONS.map((section, idx) => (
        <div key={section.label}>
          {idx > 0 && <div className="p-3 text-[10px] font-bold uppercase tracking-wider text-gray-400 mt-4">{section.label}</div>}
          {idx === 0 && <div className="p-3 text-[10px] font-bold uppercase tracking-wider text-gray-400">{section.label}</div>}
          ...
          {[1, 2, 3, 4, 5, 6, 7, 8].map(bar => (
            <div
              ...
              style={{
                ...
                animation: isPlaying ? `bounceVisualizer ${0.5 + Math.random() * 0.8}s infinite ease-in-out` : "none",
