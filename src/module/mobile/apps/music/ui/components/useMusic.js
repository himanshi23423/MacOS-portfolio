...
const useMusic = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (searchQuery.trim() !== "") {
          ...
          switch (activeCategory) {
            case "Browse": query = "Bollywood Hits"; break;
            case "Listen Now": query = "Arijit Singh Hits"; break;
            case "Hindi Music": query = "New Hindi Songs"; break;
            case "English Music": query = "Imagine Dragons"; break;
            case "Recently Added": query = "Latest Hits"; break;
            case "Artists": query = "Top Artists"; break;
            case "Albums": query = "Top Albums"; break;
            case "Songs": query = "Popular Songs"; break;
            default: query = "Bollywood Hits";
          }
        }
    ...
    const timer = setTimeout(() => { fetchTracks(); }, 450);
  ...
  const handlePlayPause = () => {
    if (activeTrack.url) { setMusicState({ isPlaying: !isPlaying }); }
  };
  ...
  const handleNext = () => {
    ...
    if (isShuffle) {
      ...
      if (currentIdx === -1) { nextTrack = currentList[0]; }
      else { nextTrack = currentList[(currentIdx + 1) % currentList.length]; }
    }
  ...
  return {
    tracks, setTracks,
    activeTrack, isPlaying, volume, isMuted,
