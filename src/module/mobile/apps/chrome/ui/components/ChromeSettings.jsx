...
import {
  Settings as SettingsIcon, User, Lock, Search, Star, History, Info,
  Sun, Moon, Trash2
} from "lucide-react";

const ChromeSettings = ({
  activeTab, navigateTabTo, settingsThemeClasses, theme,
  ...
  setHistoryList, setTabs, setActiveTabId,
  highlightText, findText
}) => {
  const { passwordsList, setPasswordsList, newPassSite, setNewPassSite,
    newPassUser, setNewPassUser, newPassVal, setNewPassVal,
    cardsList, setCardsList, newCardNum, setNewCardNum, newCardExpiry, setNewCardExpiry,
    addressesList, setAddressesList, newAddressStreet, setNewAddressStreet,
  ...
  return (
    <div className={`absolute inset-0 flex select-none overflow-hidden ${settingsThemeClasses.contentBg}`}>
      <div className={`w-52 border-r shrink-0 py-6 px-4 flex flex-col gap-1.5 ${settingsThemeClasses.sidebarBg}`}>
        ...
        <button onClick={() => setActiveSettingsSection("profile")}
        ...
        <button onClick={() => setActiveSettingsSection("autofill")}
        ...
        <button onClick={() => setActiveSettingsSection("appearance")}
        ...
        <button onClick={() => setActiveSettingsSection("search")}
        ...
        <button onClick={() => setActiveSettingsSection("privacy")}
          ...
          <div className="space-y-6">
            ...
            <div className={`p-5 rounded-xl border space-y-4 ${settingsThemeClasses.cardBg}`}>
              <div className="flex items-center gap-4">
                <div className={`w-14 h-14 ${profileColor} text-white rounded-full flex items-center justify-center text-xl font-bold shadow-md`}>K</div>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => { const n = prompt("Enter profile name:", username); if (n) setUsername(n); }}
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-bold transition-all shadow-sm">Customize profile name</button>
              </div>
            </div>
            <div className={`flex items-center justify-between p-4 rounded-xl border ${settingsThemeClasses.cardBg}`}>
              ...
              <button onClick={() => setIsSyncActive(prev => !prev)}
            ...
            <div className={`p-4 rounded-xl border space-y-3.5 ${settingsThemeClasses.cardBg}`}>
              ...
              <div className="flex items-center gap-2.5">
                {["bg-orange-600", "bg-blue-600", "bg-emerald-600", "bg-purple-600", "bg-rose-600", "bg-cyan-600"].map((c, i) => (
                  <button key={i} onClick={() => setProfileColor(c)}
        ...
        {activeSettingsSection === "appearance" && (
          <div className="space-y-6">
            ...
            <div className={`flex items-center justify-between p-4 rounded-xl border ${settingsThemeClasses.cardBg}`}>
              ...
              <button onClick={() => setShowBookmarks(prev => !prev)}
                ...
                {["small", "medium", "large"].map((size) => (
                  <button key={size} onClick={() => setFontSize(size)}
                    className={`px-3 py-1 rounded text-xs font-semibold capitalize transition-all ${
                      fontSize === size ? (theme === "dark" ? "bg-[#2f3033] text-white shadow-sm font-bold" : "bg-white text-gray-800 shadow-sm font-bold") : settingsThemeClasses.btnInactive
                    }`}>{size}</button>
                ))}
              </div>
            </div>
        ...
        {activeSettingsSection === "privacy" && (
          <div className="space-y-6">
            ...
            <div className={`flex items-center justify-between p-4 rounded-xl border ${settingsThemeClasses.cardBg}`}>
              ...
              <button onClick={() => { setHistoryList([]); setTabs([{ id: "tab-1", title: "New Tab", url: "chrome://newtab", history: ["chrome://newtab"], historyIndex: 0 }]); setActiveTabId("tab-1"); alert("History cleared!"); }}
                className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded text-xs font-bold transition-all shadow-sm cursor-pointer">Clear history</button>
            </div>
          </div>
        )}
      </div>
