...
import { Search, AlertTriangle, Globe, ArrowRight, X, Printer } from "lucide-react";
...
const ChromePageView = ({
  activeTab, theme, themeClasses, settingsThemeClasses,
  searchEngine, googleSearchQuery, setGoogleSearchQuery,
  navigateTabTo, bookmarks,
  isGitHubUrl, isLinkedInUrl, isTwitterUrl, isWikipediaUrl,
  isIframeable, isAdBlockerActive,
  isLensScanning, setIsLensScanning,
  showFindBar, showCastDialog, setShowCastDialog,
  castDevice, isCastConnecting, setIsCastConnecting,
  showQrCode, setShowQrCode,
  zoom, fontSize,
  activeSettingsSection, setActiveSettingsSection,
  username, setUsername, profileColor, setProfileColor,
  isSyncActive, setIsSyncActive,
  showBookmarks, setShowBookmarks,
  setFontSize, setSearchEngine,
  historyList, setHistoryList,
  setTabs, setActiveTabId,
  highlightText, findText,
  setDownloadsList, setAddressInput,
  passwordsList, setPasswordsList,
  newPassSite, setNewPassSite,
  newPassUser, setNewPassUser,
  newPassVal, setNewPassVal,
  cardsList, setCardsList,
  newCardNum, setNewCardNum,
  newCardExpiry, setNewCardExpiry,
  addressesList, setAddressesList,
  newAddressStreet, setNewAddressStreet,
  newAddressCity, setNewAddressCity,
}) => {
  return (
    <div
    ...
    >
      ...
      {isLensScanning && (
        <div className="absolute inset-0 bg-black/60 z-50 flex items-center justify-center pointer-events-auto">
          ...
          <div className="absolute bottom-20 bg-zinc-900/90 text-white border border-zinc-700 px-5 py-2.5 rounded-full text-xs font-semibold shadow-lg flex items-center gap-2">
            ...
            <button onClick={() => setIsLensScanning(false)} className="ml-4 text-zinc-400 hover:text-white font-bold">Cancel</button>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-black/40 z-50 flex items-center justify-center p-6 pointer-events-auto">
          <div className={`w-80 rounded-xl border p-5 shadow-2xl space-y-4 ${
            ...
            <div className="flex items-center justify-between border-b pb-2.5 border-neutral-200 dark:border-zinc-700/30">
              <span className="font-bold text-sm flex items-center gap-2"><Printer className="w-4 h-4 text-blue-500" /> Cast tab</span>
              <button onClick={() => setShowCastDialog(false)}><X className="w-4 h-4 text-zinc-400 hover:text-zinc-200" /></button>
            </div>
            {castDevice ? (
              <div className="space-y-4 text-center py-2">
                {isCastConnecting ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { setCastDevice(null); setIsCastConnecting(false); }}
                      className="px-4 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded text-xs font-bold transition-all shadow">Stop Casting</button>
                  </div>
                )}
              </div>
            ) : (
              <div className="space-y-2">
                ...
                {["Living Room TV", "Bedroom Smart TV", "Kunal's Nest Hub"].map((device, i) => (
                  <button key={i} onClick={() => { setCastDevice(device); setIsCastConnecting(true); setTimeout(() => { setIsCastConnecting(false); }, 2000); }}
        ...
        <div className="absolute inset-0 bg-black/40 z-50 flex items-center justify-center p-6 pointer-events-auto">
          <div className={`w-80 rounded-xl border p-5 shadow-2xl space-y-4 text-center ${
            ...
            <div className="flex items-center justify-between border-b pb-2.5 border-neutral-200 dark:border-zinc-700/30 text-left">
              ...
              <button onClick={() => setShowQrCode(false)}><X className="w-4 h-4 text-zinc-400 hover:text-zinc-200" /></button>
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => { alert("QR Code downloaded to device!"); setShowQrCode(false); }}
                className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-all shadow">Download</button>
              <button onClick={() => setShowQrCode(false)}
                className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition-all ${theme === "dark" ? "border-zinc-700 text-zinc-300 hover:bg-zinc-800" : "border-gray-300 text-gray-600 hover:bg-gray-50"}`}>Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className={`absolute inset-0 flex flex-col p-6 select-none overflow-y-auto ${theme === "dark" ? "bg-[#202124] text-white" : "bg-[#ffffff] text-gray-800"}`}>
          <div className="max-w-2xl w-full space-y-6">
            <div className={`flex items-center gap-4 border-b pb-4 ${theme === "dark" ? "border-gray-800" : "border-gray-150"}`}>
              <span onClick={() => navigateTabTo("chrome://newtab")}
                className={`font-bold text-2xl tracking-tight cursor-pointer select-none ${searchEngine === "Google" ? "text-blue-500" : searchEngine === "Bing" ? "text-teal-600" : "text-orange-500"}`}>{searchEngine}</span>
              <div className={`flex-1 max-w-md border rounded-full px-4 py-1.5 text-xs flex items-center ${theme === "dark" ? "bg-[#35363a] border-[#404144]" : "bg-white border-[#dadce0]"}`}>
            ...
            <div className="space-y-6 pt-2">
              <div className="space-y-1">
                ...
                <h4 onClick={() => navigateTabTo("https://en.wikipedia.org")} className="text-base text-[#1a0dab] dark:text-blue-400 hover:underline cursor-pointer font-medium leading-tight">Wikipedia - The Free Encyclopedia</h4>
                <p className={`text-xs leading-normal ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Wikipedia is a free online encyclopedia, created and edited by volunteers around the world and hosted by the Wikimedia Foundation.</p>
              </div>
              <div className="space-y-1">
                <span className="text-[10px] text-gray-500">https://openstreetmap.org</span>
                <h4 onClick={() => navigateTabTo("https://openstreetmap.org")} className="text-base text-[#1a0dab] dark:text-blue-400 hover:underline cursor-pointer font-medium leading-tight">OpenStreetMap - Free Wiki World Map</h4>
                <p className={`text-xs leading-normal ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>OpenStreetMap is a map of the world, created by people like you and free to use under an open license.</p>
              </div>
              <div className="space-y-1">
                <span className="text-[10px] text-gray-500">https://example.com</span>
                <h4 onClick={() => navigateTabTo("https://example.com")} className="text-base text-[#1a0dab] dark:text-blue-400 hover:underline cursor-pointer font-medium leading-tight">Example Domain</h4>
                <p className={`text-xs leading-normal ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>This domain is for use in illustrative examples in documents.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab.url !== "chrome://newtab" && !activeTab.url.includes("google.com/search") && !activeTab.url.startsWith("chrome://") && isIframeable(activeTab.url) && (
        <iframe
          src={activeTab.url.toLowerCase().includes("openstreetmap.org") ? "https://www.openstreetmap.org/export/embed.html" : activeTab.url}
      ...
      {/* Sandbox fallback */}
      {activeTab.url !== "chrome://newtab" && !activeTab.url.includes("google.com/search") && !activeTab.url.startsWith("chrome://") && !isGitHubUrl && !isLinkedInUrl && !isTwitterUrl && !isWikipediaUrl && !isIframeable(activeTab.url) && (
        <div className="absolute inset-0 bg-slate-50 flex items-center justify-center p-6 overflow-y-auto">
          <div className="bg-white border border-slate-200/80 p-8 rounded-2xl shadow-lg max-w-md w-full text-center space-y-6">
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              <button onClick={() => navigateTabTo("chrome://newtab")}
                className="w-full py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-xs font-bold transition-all">Back to New Tab</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-[#0d1117] text-[#c9d1d9] flex flex-col font-sans select-text overflow-y-auto z-0">
          <div className="bg-[#161b22] px-6 py-3 border-b border-[#30363d] flex items-center justify-between shrink-0 select-none">
            <div className="flex items-center gap-3">
              ...
              <div className="bg-[#0d1117] border border-[#30363d] px-3 py-1 rounded-md text-[11px] w-64 text-[#8b949e]">Search or jump to...</div>
            </div>
            <div className="flex items-center gap-4 text-xs text-[#8b949e] font-semibold">
              <span>Pull requests</span><span>Issues</span><span>Codespaces</span><span>Marketplace</span>
          ...
          <div className="flex-1 flex p-8 gap-8 max-w-6xl mx-auto w-full min-h-0">
            <div className="w-64 shrink-0 space-y-4">
              ...
              <button className="w-full py-1.5 bg-[#21262d] hover:bg-[#30363d] text-[#c9d1d9] border border-[#30363d] rounded-md font-semibold text-xs transition-colors">Follow</button>
              <p className="text-xs text-[#c9d1d9] leading-relaxed">Full Stack Developer | Building interactive macOS web portfolios, screens, and custom browser simulations.</p>
              <div className="text-xs text-[#8b949e] space-y-1.5 pt-2">
                <div className="flex items-center gap-1.5"><span>📍</span> Mumbai, India</div>
                <div className="flex items-center gap-1.5"><span>🔗</span> <a href="https://github.com/kuldeeprajput-dev" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">github.com/kuldeeprajput-dev</a></div>
              </div>
            </div>
            <div className="flex-1 space-y-6">
              <div className="flex gap-6 border-b border-[#30363d] pb-2 text-xs font-semibold text-[#8b949e] select-none">
                ...
                <span>Projects</span><span>Packages</span><span>Stars</span>
              </div>
              <div className="space-y-3">
                <h3 className="text-sm font-semibold text-white">Pinned</h3>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { name: "MacOS-portfolio", desc: "Beautiful macos desktop portfolio simulator built with React, Vite and Tailwind.", lang: "JavaScript", color: "bg-[#f1e05a]", stars: "142", forks: "28" },
                    { name: "insta-things-downloader", desc: "Clean media downloader application for Instagram posts, reels, and highlights.", lang: "TypeScript", color: "bg-[#3178c6]", stars: "89", forks: "12" },
                    { name: "resume-ats-scanner", desc: "AI powered resume screening platform comparing job description keywords against resumes.", lang: "Python", color: "bg-[#3572A5]", stars: "64", forks: "0" },
                    { name: "newtube", desc: "A modern, lightweight YouTube alternative client with ad-blocking and picture-in-picture.", lang: "JavaScript", color: "bg-[#f1e05a]", stars: "52", forks: "0" },
                  ].map((repo, idx) => (
                    <div key={idx} className="bg-[#161b22] border border-[#30363d] rounded-md p-4 space-y-3 flex flex-col justify-between hover:border-[#8b949e]/50 transition-colors">
                      <div>
                        <div className="flex items-center justify-between text-xs">
                          <span className="font-bold text-blue-400 hover:underline cursor-pointer">{repo.name}</span>
                          <span className="border border-[#30363d] px-1.5 py-0.5 rounded-full text-[9px] text-[#8b949e]">Public</span>
                        </div>
                        <p className="text-[11px] text-[#8b949e] mt-2 leading-relaxed">{repo.desc}</p>
                      </div>
                      <div className="flex items-center gap-4 text-[10px] text-[#8b949e]">
                        <span className="flex items-center gap-1"><span className={`w-3 h-3 rounded-full ${repo.color}`} /> {repo.lang}</span>
        ...
        <div className="absolute inset-0 bg-[#f3f2f0] text-gray-800 flex flex-col font-sans select-text overflow-y-auto z-0">
          <div className="bg-white border-b border-gray-200 px-6 py-2 flex items-center justify-between shrink-0 select-none shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="bg-[#edf3f8] border-none px-3 py-1.5 rounded-md text-[11px] w-64 text-gray-500 flex items-center gap-2"><span>🔍</span> Search members, jobs...</div>
            </div>
          ...
          <div className="flex-1 max-w-4xl mx-auto w-full p-6 space-y-6">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
              ...
              <div className="px-6 pb-6 relative">
                ...
                <div className="pt-20 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-full font-bold text-xs shadow-sm transition-colors">Open to</button>
                    <button className="px-4 py-1.5 border border-blue-600 text-blue-600 hover:bg-blue-50 rounded-full font-bold text-xs transition-colors">Add profile section</button>
                    <button className="px-3 py-1.5 border border-gray-400 text-gray-600 hover:bg-gray-50 rounded-full font-bold text-xs transition-colors">More</button>
                  </div>
                </div>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-3">
              ...
              <p className="text-xs leading-relaxed text-gray-700">Passionate Full-Stack Developer with 4+ years of professional experience designing, building, and launching high-performance interactive web interfaces.</p>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600 mt-2">Built stateful React components, integrated RESTful APIs, optimized asset delivery pipes.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col border-r border-zinc-800 overflow-y-auto min-w-0">
            ...
            <div className="px-6 pb-4 relative flex flex-col">
              ...
              <div className="flex justify-end pt-3">
                <button className="px-4 py-1 border border-zinc-700 hover:bg-zinc-900 rounded-full font-bold text-xs transition-colors">Edit profile</button>
              </div>
              ...
              <p className="text-xs mt-3 leading-relaxed">Full Stack Developer | Building interactive macOS portfolios. Pushing the boundaries of web UI engineering! 💻🚀 #buildinpublic</p>
            ...
            <div className="divide-y divide-zinc-800">
              {[
                { text: "Just finished building the Light Theme synchronization for my macOS portfolio! 🚀✨ #buildinpublic #webdev #reactjs", time: "2h", likes: "42", replies: "4" },
                { text: "Recursive rendering inside a mock browser in a portfolio? Type your local address in the Chrome app and see it stack! 💻🖥️ #reactjs #vite", time: "May 25", likes: "78", replies: "9" },
                { text: "Hello World! Welcome to my macOS portfolio desktop simulation. Built using React, TailwindCSS, and Lucide Icons.", time: "Jan 12", likes: "104", replies: "12" },
              ].map((tweet, idx) => (
        ...
        <div className="absolute inset-0 bg-white text-gray-900 flex flex-col font-sans select-text overflow-y-auto z-0">
          <div className="bg-white border-b border-gray-300 px-4 py-2 flex items-center justify-between shrink-0 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              <div className="bg-gray-100 border border-gray-300 px-3 py-1 rounded text-[11px] w-52 text-gray-500 flex items-center gap-1.5">
                <Search className="w-3 h-3 text-gray-400" />Search Wikipedia
              </div>
          ...
          <div className="bg-[#f8f9fa] border-b border-gray-300 px-6 flex items-center gap-0 text-[12px] shrink-0">
            {["Article", "Talk", "Read", "View source", "View history"].map((tab, i) => (
              <span key={i} className={`px-3 py-2 cursor-pointer border-b-2 ${tab === "Article" ? "border-blue-600 text-blue-600 bg-white -mb-px" : "border-transparent text-gray-600 hover:text-gray-900"}`}>{tab}</span>
            ))}
          </div>
          <div className="flex flex-1 gap-0 min-h-0">
            <div className="flex-1 overflow-y-auto p-8 max-w-4xl">
              <h1 className="text-3xl font-normal border-b border-gray-300 pb-3 mb-4">React (JavaScript library)</h1>
              ...
              <div className="float-right ml-6 mb-4 w-64 border border-gray-400 text-[12px] bg-[#f8f9fa]">
                ...
                <table className="w-full">
                  {[["Developer", "Meta Platforms"], ["Initial release", "May 29, 2013"], ["Stable release", "19.1.0 (April 2025)"], ["Written in", "JavaScript"], ["Type", "JavaScript library"], ["License", "MIT License"], ["Website", "react.dev"]].map(([k, v], i) => (
              ...
              <p className="text-[13.5px] leading-relaxed mb-4"><b>React</b> (also known as <b>React.js</b> or <b>ReactJS</b>) is a free and open-source front-end JavaScript library for building user interfaces based on components. It is maintained by Meta (formerly Facebook) and a community of individual developers and companies.</p>
              <p className="text-[13.5px] leading-relaxed mb-4">React can be used to develop single-page, mobile, or server-rendered applications with frameworks like Next.js.</p>
              <div className="border border-gray-400 bg-[#f8f9fa] inline-block p-3 mb-5 text-[12px] min-w-[200px]">
                ...
                {[["1", "History"], ["2", "Basic usage"], ["3", "Notable features"], ["3.1", "Components"], ["3.2", "Virtual DOM"], ["3.3", "JSX"], ["4", "React Hooks"], ["5", "Server-side rendering"], ["6", "See also"]].map(([num, title], i) => (
              ...
              <p className="text-[13.5px] leading-relaxed mb-4">React was created by Jordan Walke, a software engineer at Facebook. It was first deployed on Facebook's News Feed in 2011 and later on Instagram in 2012.</p>
              <h2 className="text-xl font-normal border-b border-gray-300 pb-1 mb-3">Basic usage</h2>
              ...
              <h2 className="text-xl font-normal border-b border-gray-300 pb-1 mb-3">Notable features</h2>
              <h3 className="text-[15px] font-bold mb-2">Components</h3>
              <p className="text-[13.5px] leading-relaxed mb-4">React code is made of entities called <i>components</i>. These components are reusable and must be formed in the SFC structure.</p>
              <h3 className="text-[15px] font-bold mb-2">Virtual DOM</h3>
              <p className="text-[13.5px] leading-relaxed mb-4">Another notable feature is the use of a virtual Document Object Model, or virtual DOM.</p>
              <h3 className="text-[15px] font-bold mb-2">JSX</h3>
              <p className="text-[13.5px] leading-relaxed mb-4">JSX is an extension to the JavaScript language syntax. Similar in appearance to HTML.</p>
              <div className="text-[11px] text-gray-500 border-t border-gray-300 pt-4">This page was last edited on 27 May 2025.</div>
            </div>
      ...
      {activeTab.url === "chrome://bookmarks" && (
        <div className={`absolute inset-0 flex select-none overflow-hidden ${settingsThemeClasses.contentBg}`}>
          <div className={`w-52 border-r shrink-0 py-6 px-4 flex flex-col gap-1.5 ${settingsThemeClasses.sidebarBg}`}>
            ...
            <button onClick={() => navigateTabTo("chrome://settings")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><SettingsIcon className="w-4 h-4" /> Appearance</button>
            <button className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg bg-blue-500/10 text-blue-600 dark:text-blue-400 text-left w-full"><Bookmark className="w-4 h-4" /> Bookmarks</button>
            <button onClick={() => navigateTabTo("chrome://history")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><History className="w-4 h-4" /> History</button>
            <button onClick={() => navigateTabTo("chrome://downloads")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><History className="w-4 h-4 transform rotate-180" /> Downloads</button>
            <button onClick={() => navigateTabTo("chrome://about")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><Info className="w-4 h-4" /> About Chrome</button>
          </div>
                ...
                {bookmarks.map((bookmark, index) => (
                  <div key={index} className="flex items-center justify-between p-3.5 hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <span onClick={() => navigateTabTo(bookmark.url)} className="text-xs font-semibold text-blue-500 hover:underline cursor-pointer truncate max-w-md">{highlightText(bookmark.title, findText)}</span>
                    ...
                    <button onClick={() => setBookmarks(prev => prev.filter((_, i) => i !== index))}
          ...
          <div className="max-w-xl w-full text-center space-y-6 -mt-10">
            <div className="w-20 h-20 bg-zinc-800 text-zinc-300 rounded-full flex items-center justify-center mx-auto shadow-lg border border-zinc-700">
              <svg className="w-12 h-12 text-zinc-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <circle cx="8" cy="15" r="2" /><circle cx="16" cy="15" r="2" /><path d="M10 15h4M2 11a5 5 0 0 1 5-5h10a5 5 0 0 1 5 5v1H2z" />
              </svg>
            </div>
            <div className="space-y-2">
              <h2 className="text-2xl font-bold text-zinc-100">You've gone Incognito</h2>
              <p className="text-xs text-zinc-400 max-w-md mx-auto leading-relaxed">Now you can browse privately, and other people who use this device won't see your activity.</p>
            </div>
            <div className="bg-zinc-800/60 border border-zinc-700/50 rounded-xl p-4 text-left text-[10px] text-zinc-300 grid grid-cols-2 gap-4">
              <div>
                ...
                <ul className="list-disc pl-4 space-y-1 text-zinc-400"><li>Your browsing history</li><li>Cookies and site data</li><li>Information entered in forms</li></ul>
              </div>
              <div>
                <h4 className="font-bold text-zinc-100 mb-1">Your activity might still be visible to:</h4>
                <ul className="list-disc pl-4 space-y-1 text-zinc-400"><li>Websites you visit</li><li>Your employer or school</li><li>Your internet service provider</li></ul>
              </div>
            </div>
            <div className="w-full flex items-center bg-zinc-800 border border-zinc-700 hover:border-transparent hover:shadow-md focus-within:shadow-md focus-within:border-transparent transition-all rounded-full px-4 py-3 text-sm max-w-md mx-auto">
              ...
              <input type="text" placeholder="Search Google privately"
                onKeyDown={(e) => { if (e.key === "Enter" && e.target.value.trim()) { navigateTabTo(`https://www.google.com/search?q=${encodeURIComponent(e.target.value)}`); e.target.value = ""; } }}
      ...
      {activeTab.url === "chrome://downloads" && (
        <div className={`absolute inset-0 flex select-none overflow-hidden ${settingsThemeClasses.contentBg}`}>
          <div className={`w-52 border-r shrink-0 py-6 px-4 flex flex-col gap-1.5 ${settingsThemeClasses.sidebarBg}`}>
            ...
            <button onClick={() => navigateTabTo("chrome://settings")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><SettingsIcon className="w-4 h-4" /> Appearance</button>
            <button onClick={() => navigateTabTo("chrome://bookmarks")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><Star className="w-4 h-4" /> Bookmarks</button>
            <button onClick={() => navigateTabTo("chrome://history")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><History className="w-4 h-4" /> History</button>
            <button className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg bg-blue-500/10 text-blue-600 dark:text-blue-400 text-left w-full"><History className="w-4 h-4 transform rotate-180" /> Downloads</button>
            <button onClick={() => navigateTabTo("chrome://about")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><Info className="w-4 h-4" /> About Chrome</button>
          </div>
          <div className="flex-1 overflow-y-auto p-8 max-w-3xl space-y-6">
            <div className={`flex items-center justify-between border-b ${settingsThemeClasses.borderMuted} pb-4`}>
              ...
              {downloadsList.length > 0 && (
                <button onClick={() => setDownloadsList([])} className="text-xs font-semibold text-rose-500 hover:bg-rose-500/10 px-3 py-1.5 rounded-lg transition-colors">Clear All</button>
              )}
            </div>
            <div className="space-y-4">
              {downloadsList.map((d, index) => (
                <div key={index} className={`p-4 rounded-xl border flex items-center justify-between gap-4 ${settingsThemeClasses.cardBg}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setDownloadsList(prev => prev.filter((_, i) => i !== index))}
                      className="px-3 py-1 hover:bg-rose-500/10 text-rose-500 dark:text-rose-400 border border-rose-200 dark:border-rose-900/30 rounded text-[10px] font-semibold cursor-pointer">Remove</button>
                  </div>
                </div>
              ))}
              {downloadsList.length === 0 && (
          ...
          <div className="flex-1 overflow-y-auto p-8 grid grid-cols-2 gap-6 max-w-4xl">
            <div className={`p-5 rounded-xl border flex flex-col justify-between gap-4 shadow-sm ${settingsThemeClasses.cardBg}`}>
              <div className="space-y-2">
                ...
                <p className={`text-[10px] ${settingsThemeClasses.textMuted} leading-relaxed`}>Blocks intrusive advertisements, tracking cookies, and popup marketing banners across all websites.</p>
              </div>
            ...
            <div className={`p-5 rounded-xl border flex flex-col justify-between gap-4 shadow-sm ${settingsThemeClasses.cardBg}`}>
              <div className="space-y-2">
                ...
                <p className={`text-[10px] ${settingsThemeClasses.textMuted} leading-relaxed`}>Adds React component debugging tools and state inspector consoles.</p>
              </div>
              <div className={`flex items-center justify-between pt-2 border-t ${settingsThemeClasses.borderMuted}`}>
                <span className={`text-[10px] ${settingsThemeClasses.textMuted}`}>Enable extension</span>
                <button className="w-10 h-5.5 rounded-full p-0.5 bg-blue-600"><div className="w-4 h-4 rounded-full bg-white translate-x-4.5" /></button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 bg-[#202124] text-gray-200 flex select-none overflow-hidden font-mono">
          ...
          <div className="w-1/2 flex flex-col p-4 overflow-y-auto justify-between bg-zinc-950">
            ...
            <div className="border-t border-zinc-800 pt-2 flex items-center text-[10px] text-zinc-400 gap-1.5">
              <span>&gt;</span>
              <input type="text" placeholder="Enter console command..."
                onKeyDown={(e) => { if (e.key === "Enter" && e.target.value.trim()) { alert(`Console command executed: "${e.target.value}"`); e.target.value = ""; } }}
      ...
      {activeTab.url === "chrome://about" && (
        <div className={`absolute inset-0 flex select-none overflow-hidden ${settingsThemeClasses.contentBg}`}>
          <div className={`w-52 border-r shrink-0 py-6 px-4 flex flex-col gap-1.5 ${settingsThemeClasses.sidebarBg}`}>
            ...
            <button onClick={() => navigateTabTo("chrome://history")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><History className="w-4 h-4" /> History</button>
            <button onClick={() => navigateTabTo("chrome://downloads")} className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-left hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"><History className="w-4 h-4 transform rotate-180" /> Downloads</button>
            <button className="flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg bg-blue-500/10 text-blue-600 dark:text-blue-400 text-left"><Info className="w-4 h-4" /> About Chrome</button>
          </div>
          <div className="flex-1 overflow-y-auto p-8 max-w-2xl space-y-6">
            ...
            <div className={`space-y-4 text-xs leading-relaxed ${settingsThemeClasses.textMuted}`}>
              <p className="font-semibold text-gray-700 dark:text-gray-300">Google Chrome is up to date</p>
              <p>This mockup browser runs inside the Antigravity MacOS portfolio interface.</p>
              <p className="text-[10px] text-gray-400 pt-4">Copyright 2026 Google DeepMind. All rights reserved.</p>
            </div>
          </div>
        </div>
      )}
