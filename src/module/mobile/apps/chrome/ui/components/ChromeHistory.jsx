    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-3xl space-y-6">
        ...
        {historyList.length === 0 ? (
          ...
          <div
            className={`border rounded-xl divide-y overflow-hidden shadow-sm ${theme === "dark" ? "bg-[#2f3033] border-[#3c3e41] divide-[#3c3e41]" : "bg-white border-gray-200 divide-gray-200"}`}
          >
            {historyList.map((item, index) => (
              ...
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <span className={`text-[10px] ${settingsThemeClasses.textMuted} shrink-0 w-12`}>
                    {item.time}
                  </span>
                  ...
                  >
                    {highlightText(item.url, findText)}
                  </span>
                </div>
                <button
