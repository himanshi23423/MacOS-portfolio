...
import { History, Settings as SettingsIcon, Star, Info, Globe, Trash2 } from "lucide-react";

const ChromeHistory = ({
  activeTab, navigateTabTo, settingsThemeClasses, theme,
  historyList, setHistoryList, highlightText, findText
}) => {
  if (activeTab.url !== "chrome://history") return null;
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-3xl space-y-6">
        <div className={`flex items-center justify-between border-b ${settingsThemeClasses.borderMuted} pb-4`}>
          ...
          {historyList.length > 0 && (
            <button onClick={() => setHistoryList([])}
              className="px-3 py-1.5 text-rose-500 hover:bg-rose-500/10 rounded-lg text-xs font-semibold transition-all cursor-pointer">Clear History</button>
          )}
        </div>
            ...
            {historyList.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-3.5 hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <span onClick={() => navigateTabTo(item.url)}
                    className="text-xs font-semibold text-blue-500 hover:underline cursor-pointer truncate max-w-md">{highlightText(item.title, findText)}</span>
                  <span className={`text-[10px] ${settingsThemeClasses.textMuted} truncate hidden md:inline`}>{highlightText(item.url, findText)}</span>
                </div>
                <button onClick={() => setHistoryList(prev => prev.filter((_, i) => i !== index))}
