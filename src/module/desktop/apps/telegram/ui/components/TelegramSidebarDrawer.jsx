...
import {
  ArrowLeft, User, Users, Megaphone, Phone, Bookmark,
  Settings as SettingsIcon, Moon, Camera, Lock, Bell, Monitor
} from "lucide-react";
...
const TelegramSidebarDrawer = ({
  ...
  handleCreateGroup, handleCreateChannel,
  openSavedMessages, setActiveChatId, setNightMode
}) => {
        ...
        {drawerSection === "menu" && (
          <div className="flex flex-col h-full">
            <div className="p-5 text-left flex flex-col items-start gap-3 bg-gradient-to-br from-indigo-500/10 via-blue-500/5 to-cyan-500/10 shrink-0">
              <div className="w-14 h-14 rounded-full bg-orange-500 text-white font-bold text-lg flex items-center justify-center shadow-md">P</div>
              <div>
                <h4 className={`font-bold text-sm flex items-center gap-1 ${nightMode ? "text-white" : "text-gray-900"}`}>
                  {userProfile.name}
                </h4>
                <span className={`text-[10px] ${nightMode ? "text-zinc-400" : "text-gray-500"}`}>Set Emoji Status</span>
              </div>
            </div>

            <div className="p-2 space-y-0.5 text-xs text-left">
              <button onClick={() => setDrawerSection("profile")} className="w-full p-2.5 rounded-lg flex items-center gap-3 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-850">
                <User className="w-4.5 h-4.5 text-gray-400" />
                <span>My Profile</span>
              </button>
              <button onClick={() => setDrawerSection("new_group")} className="w-full p-2.5 rounded-lg flex items-center gap-3 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-850">
                <Users className="w-4.5 h-4.5 text-gray-400" />
                <span>New Group</span>
              </button>
              <button onClick={() => setDrawerSection("new_channel")} className="w-full p-2.5 rounded-lg flex items-center gap-3 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-850">
                <Megaphone className="w-4.5 h-4.5 text-gray-400" />
                <span>New Channel</span>
              </button>
              <button onClick={() => setDrawerSection("contacts")} className="w-full p-2.5 rounded-lg flex items-center gap-3 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-850">
                <User className="w-4.5 h-4.5 text-gray-400" />
                <span>Contacts</span>
              </button>
              <button onClick={() => setDrawerSection("calls")} className="w-full p-2.5 rounded-lg flex items-center gap-3 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-850">
                <Phone className="w-4.5 h-4.5 text-gray-400" />
                <span>Calls</span>
              </button>
              <button onClick={openSavedMessages} className="w-full p-2.5 rounded-lg flex items-center gap-3 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-850">
                <Bookmark className="w-4.5 h-4.5 text-gray-400" />
                <span>Saved Messages</span>
              </button>
              <button onClick={() => setDrawerSection("settings")} className="w-full p-2.5 rounded-lg flex items-center gap-3 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-850">
          ...
          <div className="p-4 space-y-4 text-xs text-left">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={userProfile.name} onChange={(e) => setUserProfile(prev => ({ ...prev, name: e.target.value }))}
              ...
              <div>
                ...
                <input type="text" value={userProfile.username} onChange={(e) => setUserProfile(prev => ({ ...prev, username: e.target.value }))}
                  className="w-full bg-zinc-100 dark:bg-zinc-800 dark:text-white border-none rounded px-3 py-1.5 mt-1 outline-none text-xs focus:ring-1 focus:ring-blue-500 select-text" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase">Phone Number</label>
                <input type="text" value={userProfile.phone} onChange={(e) => setUserProfile(prev => ({ ...prev, phone: e.target.value }))}
                  className="w-full bg-zinc-100 dark:bg-zinc-800 dark:text-white border-none rounded px-3 py-1.5 mt-1 outline-none text-xs focus:ring-1 focus:ring-blue-500 select-text" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase">Bio</label>
                <textarea rows="3" value={userProfile.bio} onChange={(e) => setUserProfile(prev => ({ ...prev, bio: e.target.value }))}
        ...
        {drawerSection === "new_group" && (
          <div className="p-4 space-y-4 text-xs text-left">
            ...
            <div>
              ...
              <input type="text" placeholder="e.g. Next.js Developers" value={newGroupName}
            ...
            <button onClick={handleCreateGroup} className="w-full py-2 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg transition-colors cursor-pointer text-center">
              Create Group
            </button>
          </div>
        )}
          ...
          <div className="p-4 space-y-4 text-xs text-left">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" placeholder="e.g. Daily Tech Bytes" value={newChannelName}
                  onChange={(e) => setNewChannelName(e.target.value)}
                  className="w-full bg-zinc-100 dark:bg-zinc-800 dark:text-white border border-zinc-200 dark:border-zinc-700 rounded px-3 py-1.5 mt-1 outline-none focus:border-blue-500 select-text" />
              </div>
              <div>
                <label className="text-[10px] text-gray-400 font-medium">Description</label>
                <textarea placeholder="What is this channel about?" rows="3" value={newChannelBio}
                  onChange={(e) => setNewChannelBio(e.target.value)}
                  className="w-full bg-zinc-100 dark:bg-zinc-800 dark:text-white border border-zinc-200 dark:border-zinc-700 rounded px-3 py-1.5 mt-1 outline-none focus:border-blue-500 select-text" />
              </div>
            </div>
            <button onClick={handleCreateChannel} className="w-full py-2 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg transition-colors cursor-pointer text-center">
              Create Channel
            </button>
          </div>
        )}
        ...
        {drawerSection === "contacts" && (
          <div className="p-2 space-y-1 text-xs text-left">
            ...
            {[
              ...
              <div key={cont.name} onClick={() => { setActiveChatId(cont.id); setIsDrawerOpen(false); }}
                className="p-2 flex items-center gap-3 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-850 cursor-pointer">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white ${cont.color}`}>{cont.initial}</div>
                <div>
            ...
            <div>
              ...
              <div className="flex gap-2.5">
                {["blue", "green", "purple", "orange"].map(col => (
                  <button key={col} onClick={() => setChatThemeColor(col)}
                    className={`w-6 h-6 rounded-full border-2 transition-all cursor-pointer ${
                      col === "blue" ? "bg-blue-500" : col === "green" ? "bg-emerald-500" : col === "purple" ? "bg-purple-500" : "bg-orange-500"
