...
import { 
  Moon, Plus, ToggleLeft, ToggleRight, ArrowLeft, 
  User, Grid, Clock, ChevronRight, X, Briefcase, 
  BookOpen, Gamepad2, Heart, PlusCircle, Check
} from "lucide-react";
...
const SettingsFocusSection = () => {
  ...
  const [focusProfiles, setFocusProfiles] = useState([
    { id: "dnd", name: "Do Not Disturb", desc: "Silences notifications", active: false, icon: Moon, color: "bg-indigo-500 text-white" },
    { id: "work", name: "Work", desc: "Turned on during office hours", active: false, icon: Briefcase, color: "bg-orange-500 text-white" },
    { id: "gaming", name: "Gaming", desc: "Silences alerts during full screen play", active: false, icon: Gamepad2, color: "bg-red-500 text-white" }
  ]);
  ...
  const focusTemplates = [
    { name: "Personal", icon: Heart, color: "bg-purple-500 text-white", desc: "Silences notifications when off duty" },
    { name: "Reading", icon: BookOpen, color: "bg-teal-500 text-white", desc: "Minimize distractions during study" }
  ];

  const handleToggleDnd = () => {
    toggleSystemSetting("focusMode");
    // Keep profiles active synced with DND
    setFocusProfiles(prev => prev.map(p => p.id === "dnd" ? { ...p, active: !focusMode } : p));
  };
      ...
      <div className="max-w-2xl mx-auto p-8 animate-in fade-in slide-in-from-left-2 duration-200">
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-150">
          <div className="flex items-center gap-3.5">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg ${profile.color}`}>
              <IconComp size={20} />
            </div>
            <div>
              <h2 className="text-base font-extrabold text-gray-900 leading-tight">{profile.name}</h2>
        ...
        <div className="space-y-6 animate-in fade-in duration-200">
          <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-wider ml-1">Allowed Notifications</h3>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5 min-h-[50px] items-start content-start">
                {allowedPeople.map((person) => (
                  <span key={person} className="bg-gray-100 border border-gray-200 rounded px-2 py-0.5 text-[11px] font-semibold text-gray-700 flex items-center gap-1">
                    {person}
                    <button onClick={() => setAllowedPeople(prev => prev.filter(p => p !== person))} className="text-gray-400 hover:text-gray-600">
                      <X size={10} />
                    </button>
                  </span>
                ))}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5 min-h-[50px] items-start content-start">
                {allowedApps.map((app) => (
                  <span key={app} className="bg-gray-100 border border-gray-200 rounded px-2 py-0.5 text-[11px] font-semibold text-gray-700 flex items-center gap-1">
                    {app}
                    <button onClick={() => setAllowedApps(prev => prev.filter(a => a !== app))} className="text-gray-400 hover:text-gray-600">
                      <X size={10} />
                    </button>
                  </span>
                ))}
          ...
          {/* Schedule Section */}
          <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-wider ml-1">Schedule & Automation</h3>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm text-xs text-gray-700 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[11px] text-gray-400 font-semibold block mt-0.5">Automatically trigger Focus from 22:00 to 07:00 daily</span>
              </div>
            </div>
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 animate-in fade-in slide-in-from-bottom-2 duration-300 relative min-h-[440px]">
      ...
      {/* Focus Modes List */}
      <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-wider ml-1 mb-2">Focus Profiles</h3>
      <div className="w-full bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm divide-y divide-gray-100 mb-6">
        {focusProfiles.map((profile) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${profile.color}`}>
                  <IconComp size={16} />
                </div>
              ...
              <div className="flex items-center gap-2">
                <span className={`text-[12px] font-semibold ${isActive ? "text-[#007aff]" : "text-gray-400"}`}>
                  {isActive ? "On" : "Off"}
                </span>
      ...
      <div className="w-full bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm p-4 mb-6 text-xs text-gray-700">
        <div className="flex items-center justify-between">
          <div>
            ...
            <span className="text-[10.5px] text-gray-400 font-semibold block mt-0.5">Sync focus statuses with all device profiles on this iCloud</span>
          </div>
          <button 
        ...
        <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-xl shadow-2xl p-5 w-80 max-w-full select-none animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
              <h4 className="text-[13px] font-extrabold text-gray-900 leading-tight">Create Focus Profile</h4>
              <button onClick={() => setShowAddModal(false)} className="text-gray-400 hover:text-gray-600">
                <X size={15} />
              </button>
            </div>
            
            <p className="text-[11px] text-gray-400 font-semibold mb-3">Choose a template for your new focus filter:</p>
              ...
              {focusTemplates.map((template) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <span className="text-[12px] font-bold text-gray-800 block leading-tight">{template.name}</span>
                      <span className="text-[9.5px] text-gray-400 font-semibold block leading-tight mt-0.5">{template.desc}</span>
                    </div>
                  </div>
                );
              })}
