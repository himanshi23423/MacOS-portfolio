  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 @sm:p-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex flex-col items-center mb-6 @sm:mb-8 text-center px-2 overflow-hidden w-full">
        <img src={githubData.profile.avatar_url} className="w-20 h-20 @sm:w-24 @sm:h-24 rounded-full mb-3 @sm:mb-4 shadow-lg border-2 border-white shrink-0" alt="Avatar" />
        <h2 className="text-[20px] @sm:text-[24px] font-semibold text-gray-900 tracking-tight break-words max-w-full">{githubData.profile.name || githubData.profile.login}</h2>
        <p className="text-[12px] @sm:text-[13px] text-gray-500 mt-1 break-all @sm:break-words max-w-full px-2">{githubData.profile.email || `${githubData.profile.login}@users.noreply.github.com`}</p>
      </div>
      <div className="space-y-6 w-full">
        ...
        <div className="w-full bg-gray-50 rounded-xl border border-gray-200 overflow-hidden shadow-sm">
          <div className="flex items-center justify-between p-3 px-4 border-b border-gray-200 gap-4">
            ...
            <span className="text-[13px] font-medium text-gray-900 text-right break-words flex-1 min-w-0">{githubData.profile.location || "Earth"}</span>
          </div>
          <div className="flex items-center justify-between p-3 px-4 border-b border-gray-200 gap-4">
            <span className="text-[13px] text-gray-700 shrink-0">Public Repositories</span>
            <span className="text-[13px] font-medium text-gray-900 text-right flex-1 min-w-0">{githubData.profile.public_repos}</span>
          </div>
          <div className="flex items-center justify-between p-3 px-4 gap-4">
            ...
            <span className="text-[13px] font-medium text-gray-900 text-right flex-1 min-w-0">{githubData.profile.followers}</span>
          </div>
        </div>
        ...
        <div className="w-full bg-gray-50 rounded-xl border border-gray-200 overflow-hidden shadow-sm">
          {githubData.repos.length > 0 ? githubData.repos.map((repo, i) => (
            ...
            >
              ...
              <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400 shrink-0 ml-2"><polyline points="9 18 15 12 9 6" /></svg>
            </div>
          )) : (
