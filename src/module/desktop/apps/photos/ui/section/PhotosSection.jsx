...
import { 
  Search, Plus, Share2, RotateCw, Trash2, Info, 
  LayoutGrid, Rows3, SlidersHorizontal, ZoomIn, ZoomOut,
  ChevronLeft, ChevronRight, X, MapPin, Mail, MessageSquare,
  Send, FileText, FolderPlus
} from "lucide-react";
...
const PhotosSection = ({
  ...
  const handlePrevPhoto = currentIdx > 0 ? () => setInAppViewerPhoto(filteredGallery[currentIdx - 1]) : null;
  const handleNextPhoto = currentIdx < filteredGallery.length - 1 ? () => setInAppViewerPhoto(filteredGallery[currentIdx + 1]) : null;
  ...
  return (
    <div className="flex flex-col h-full w-full @container bg-white rounded-xl overflow-hidden select-none relative">
      ...
      <div id="window-header" className="shrink-0 !bg-[#f5f5f5] !border-b-[#d4d4d4] !px-3 !py-1.5 flex flex-col gap-1 z-20">
        {/* Top toolbar row */}
          ...
          <div className="flex items-center gap-0.5 relative pr-2">
            <button 
              ...
              className={`p-1 rounded transition-colors ${
                selectedPhoto ? "hover:bg-black/5 text-gray-500" : "text-gray-300 cursor-not-allowed"
              }`}
              title="Rotate 90°"
            >
              <RotateCw size={13} />
            ...
            <div className="relative" ref={shareDropdownRef}>
              ...
              {showShareDropdown && (
                <div 
                  ...
                  onMouseDown={(e) => e.stopPropagation()}
                >
                  <button onClick={() => handleShareOption("Mail")} className="w-full text-left px-3 py-1.5 hover:bg-blue-500 hover:text-white flex items-center gap-2">
                    <Mail size={13} className="text-sky-500 group-hover:text-white" /> Mail
                  </button>
                  <button onClick={() => handleShareOption("Messages")} className="w-full text-left px-3 py-1.5 hover:bg-blue-500 hover:text-white flex items-center gap-2">
                    <MessageSquare size={13} className="text-green-500" /> Messages
                  </button>
                  <button onClick={() => handleShareOption("AirDrop")} className="w-full text-left px-3 py-1.5 hover:bg-blue-500 hover:text-white flex items-center gap-2">
                    <Send size={13} className="text-blue-500" /> AirDrop
                  </button>
                  <button onClick={() => handleShareOption("Notes")} className="w-full text-left px-3 py-1.5 hover:bg-blue-500 hover:text-white flex items-center gap-2">
                    <FileText size={13} className="text-yellow-500" /> Add to Notes
                  </button>
                  <div className="border-t border-gray-100 my-1" />
                  <button onClick={() => handleShareOption("Shared Album")} className="w-full text-left px-3 py-1.5 hover:bg-blue-500 hover:text-white flex items-center gap-2">
        ...
        {showInfo && selectedPhoto && (
          <div className="w-60 flex-shrink-0 bg-[#f5f5f5]/90 backdrop-blur-md border-l border-[#d4d4d4] flex flex-col p-4 overflow-y-auto select-none">
            ...
            <div className="space-y-3 border-t border-gray-200/80 pt-3 text-[11px]">
              <div>
                <span className="text-gray-400 font-medium block text-[9px] uppercase">Date Taken</span>
                <span className="text-gray-700 font-semibold">{selectedPhoto.date}</span>
              </div>
              <div>
                <span className="text-gray-400 font-medium block text-[9px] uppercase">Location</span>
                <span className="text-gray-700 font-semibold">{selectedPhoto.location}</span>
              </div>
              <div>
                <span className="text-gray-400 font-medium block text-[9px] uppercase">Resolution & Size</span>
                <span className="text-gray-700 font-semibold">{selectedPhoto.resolution} • {selectedPhoto.size}</span>
              </div>
            </div>
            ...
            <div className="mt-4 border-t border-gray-200/80 pt-3">
              <span className="text-[9px] font-bold text-gray-400 uppercase block mb-1.5">Map Location</span>
              <div className="w-full h-24 rounded bg-sky-100/50 border border-sky-200 relative overflow-hidden flex flex-col items-center justify-center text-center p-1">
                ...
                <span className="text-[9px] text-sky-800 font-bold z-10 truncate w-full mt-1 bg-white/80 px-1 py-0.5 rounded">{selectedPhoto.location}</span>
              </div>
            </div>
          </div>
        )}
