...
const PostmanSidebar = ({
  ...
  <div className="w-56 bg-[#f8f9fa] border-r border-zinc-200 flex flex-col shrink-0 min-w-0 font-sans select-none">
    {/* Tab switches */}
    <div className="flex border-b border-zinc-200 text-xs text-gray-500 font-semibold shrink-0">
      <button
        ...
        className={`flex-1 py-2 text-center border-b-2 flex items-center justify-center gap-1.5 transition-colors cursor-pointer ${
          sidebarTab === "collections" ? "border-orange-500 text-orange-600 bg-white" : "border-transparent hover:bg-gray-100 hover:text-gray-700"
        }`}
      >
        <Layers size={11} />
        Collections
      ...
      <button
        ...
        className={`flex-1 py-2 text-center border-b-2 flex items-center justify-center gap-1.5 transition-colors cursor-pointer ${
          sidebarTab === "history" ? "border-orange-500 text-orange-600 bg-white" : "border-transparent hover:bg-gray-100 hover:text-gray-700"
        }`}
      >
        <History size={11} />
        History
                ...
                {isExpanded && (
                  <div className="pl-3 border-l border-zinc-200/70 ml-3.5 space-y-0.5">
                    {folder.requests.map(req => {
                      const isActive = url.includes(req.url.replace("https://api.dev", "")) && method === req.method;
                      return (
                        <div
                          key={req.id}
                          ...
                          className={`flex items-center gap-1.5 py-1 px-2 hover:bg-gray-200/60 rounded cursor-pointer text-[11px] font-medium transition-colors ${
                            isActive ? "bg-gray-200/80 text-gray-900 border-r-2 border-orange-500" : "text-gray-600"
                          }`}
                        >
                          <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded border scale-95 shrink-0 ${getMethodColor(req.method)}`}>
                            {req.method}
                          </span>
        ...
        <div className="flex-1 overflow-y-auto p-1.5 space-y-1">
          {history.length > 0 ? (
            history.map((h, index) => (
              <div
                key={index}
                onClick={() => loadRequest({ method: h.method, url: h.url, body: "" })}
              ...
              >
                <div className="flex items-center gap-1.5">
                  <span className={`text-[7px] font-bold px-1 py-0.2 rounded border scale-95 shrink-0 ${getMethodColor(h.method)}`}>
                    {h.method}
                  </span>
            ...
            <div className="text-xs text-gray-400 italic text-center p-4">No request history yet.</div>
          )}
        </div>
      </>
    )}
