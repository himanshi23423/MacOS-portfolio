...
const CallDialPad = ({ dialNumber, onDialPress, onBackspace, onClear, onInitiateCall, onInputChange }) => {
  return (
    <div className="flex-1 flex flex-col p-4 justify-between min-h-0 overflow-y-auto thin-scrollbar">
      <div className="flex items-center justify-between bg-gray-200/50 border border-gray-300/20 rounded-xl p-3 mb-4 shrink-0">
        ...
        />
        {dialNumber && (
          <button onClick={onBackspace} className="p-1 hover:bg-gray-200 rounded-full text-gray-500" title="Backspace">
            <X className="w-4 h-4" />
          </button>
        )}
      </div>
      ...
      <div className="flex items-center justify-center gap-3 pt-6 shrink-0">
        <button
          onClick={() => { if (dialNumber) { onInitiateCall(dialNumber, "audio"); } }}
          disabled={!dialNumber}
        ...
        <button
          onClick={() => { if (dialNumber) { onInitiateCall(dialNumber, "video"); } }}
          disabled={!dialNumber}
        ...
        {dialNumber && (
          <button onClick={onClear} className="p-3 bg-red-100 hover:bg-red-200 rounded-full text-red-500 transition-colors" title="Clear All">
